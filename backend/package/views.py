...
def detail(request, id):
    try:
        ...
        item = Package.objects.get(pk=id)
        readme_relevant_ids = improved_recommendations_2(id, 'readme')
        name_des_relevant_ids = improved_recommendations_2(id, 'name_des')
        readme_relevant_packages = Package.objects.filter(pk__in=readme_relevant_ids)
        name_des_relevantpackages = Package.objects.filter(pk__in=name_des_relevant_ids)
        return JsonResponse({
